...
xpath = {
    ...
    "only_message": "//div/div/div[2]/div[4]/div/ul/li[{pos}]/div/div[2]/p"
}
...
if len(all_ids) != driver.get_elements_size(xpath["number_conversations"])-1:
    for i in range(1, driver.get_elements_size(xpath["number_conversations"])-1):
        ...
        if id not in all_ids:
            ...
            if count_messages_linkedin == len(messages_database):
                ...
                for i in range(len(messages_database), driver.get_elements_size(xpath["number_messages"])+1):
                    try:
                        sender = driver.get_text_from_xpath(
                            xpath["sender"].format(pos=i))
                        time = driver.get_text_from_xpath(
